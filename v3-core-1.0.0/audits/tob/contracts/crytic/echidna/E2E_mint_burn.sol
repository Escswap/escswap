...
import '../../../../../contracts/EscswapV3Pool.sol';
...
contract E2E_mint_burn {
    ...
    EscswapMinter minter;
    EscswapSwapper swapper;
    ...
    constructor() public {
        ...
        escswap = new SetupEscswap(token0, token1);

        minter = new EscswapMinter(token0, token1);
        swapper = new EscswapSwapper(token0, token1);
    ...
    function check_mint_invariants(
        int24 _tickLower,
        int24 _tickUpper,
        EscswapMinter.MinterStats memory bfre,
        EscswapMinter.MinterStats memory aftr
    ) internal {
        (, int24 currentTick, , , , , ) = pool.slot0();
    ...
    function _init(uint128 _seed) internal {
        ...
        // deploy the pool
        //
        escswap.createPool(poolParams.fee, poolParams.startPrice);
        pool = escswap.pool();
    ...
    function test_mint(uint128 _amount) public {
        ...
        (EscswapMinter.MinterStats memory bfre, EscswapMinter.MinterStats memory aftr) =
    ...
    function test_burn_partial(uint128 _amount) public {
        ...
        try minter.doBurn(pos.tickLower, pos.tickUpper, burnAmount) returns (
            EscswapMinter.MinterStats memory bfre_burn,
            EscswapMinter.MinterStats memory aftr_burn
        ) {
            bfre = bfre_burn;
            aftr = aftr_burn;
        } catch {
