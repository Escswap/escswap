...
import { TestEscswapV3SwapPay } from '../typechain/TestEscswapV3SwapPay'
...
import { TestEscswapV3ReentrantCallee } from '../typechain/TestEscswapV3ReentrantCallee'
...
describe('EscswapV3Pool', () => {
  ...
  let factory: EscswapV3Factory
  let pool: MockTimeEscswapV3Pool

  let swapTarget: TestEscswapV3Callee
  ...
  async function initializeAtZeroTick(pool: MockTimeEscswapV3Pool): Promise<void> {
    await pool.initialize(encodePriceSqrt(1, 1))
  ...
  // https://github.com/Escswap/escswap-v3-core/issues/214
  ...
  describe('#lock', () => {
    ...
    it('cannot reenter from swap callback', async () => {
      const reentrant = (await (
        await ethers.getContractFactory('TestEscswapV3ReentrantCallee')
      ).deploy()) as TestEscswapV3ReentrantCallee
  ...
  describe('swap underpayment tests', () => {
    let underpay: TestEscswapV3SwapPay
    beforeEach('deploy swap test', async () => {
      const underpayFactory = await ethers.getContractFactory('TestEscswapV3SwapPay')
      underpay = (await underpayFactory.deploy()) as TestEscswapV3SwapPay
