...
import { EscswapInterfaceMulticall } from '../src/abi/types'
...
export function useContract(chainId: ChainId) {
  return useMemo(() => {
    return new Contract(MULTICALL_ADDRESS, MulticallABI, getProvider(chainId)) as EscswapInterfaceMulticall
  }, [])
}
...
export function useMaxTokenBalance(chainId: ChainId, blockNumber: number | undefined): string | undefined {
  const { contracts, accounts } = useMemo(
    () => ({
      // The first element is intentionally empty to test sparse arrays; see https://github.com/Escswap/redux-multicall/pull/21.
