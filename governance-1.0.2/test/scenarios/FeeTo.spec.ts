...
import EscswapV2Pair from '@escswap/v2-core/build/EscswapV2Pair.json'
...
describe('scenario:FeeTo', () => {
  ...
  beforeEach('deploy escswap v2', async () => {
    factory = await deployContract(wallet, EscswapV2Factory, [wallet.address])
  })
  ...
  describe('tokens', () => {
    ...
    beforeEach('make test tokens', async () => {
      const { timestamp: now } = await provider.getBlock('latest')
      const token0 = await deployContract(wallet, Esc, [wallet.address, constants.AddressZero, now + 60 * 60])
      tokens.push(token0)
      const token1 = await deployContract(wallet, Esc, [wallet.address, constants.AddressZero, now + 60 * 60])
      tokens.push(token1)
    })
    ...
    beforeEach('create fee liquidity', async () => {
      ...
      pair = new Contract(pairAddress, EscswapV2Pair.abi).connect(wallet)
