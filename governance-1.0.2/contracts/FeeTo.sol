...
contract FeeTo {
    ...
    function renounce(address pair) public returns (uint value) {
        ...
        TokenAllowState storage token0AllowState = tokenAllowStates[IEscswapV2Pair(pair).token0()];
        TokenAllowState storage token1AllowState = tokenAllowStates[IEscswapV2Pair(pair).token1()];
        ...
        if (
            ...
            value = IEscswapV2Pair(pair).balanceOf(address(this));
            if (value > 0) {
                ...
                assert(IEscswapV2Pair(pair).transfer(pair, value));
                IEscswapV2Pair(pair).burn(pair);
            }
    ...
    function claim(address pair) public returns (uint value) {
        ...
        if (
            ...
            if (value > 0) {
                ...
                assert(IEscswapV2Pair(pair).transfer(feeRecipient, value));
            }
        }
    }
}

interface IEscswapV2Pair {
