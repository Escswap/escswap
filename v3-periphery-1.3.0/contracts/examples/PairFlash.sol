...
import '@escswap/v3-core/contracts/interfaces/callback/IEscswapV3FlashCallback.sol';
...
/// @notice An example contract using the Escswap V3 flash function
contract PairFlash is IEscswapV3FlashCallback, PeripheryPayments {
    ...
    /// @notice Calls the pools flash function with data needed in `escswapV3FlashCallback`
    function initFlash(FlashParams memory params) external {
        PoolAddress.PoolKey memory poolKey =
            PoolAddress.PoolKey({token0: params.token0, token1: params.token1, fee: params.fee1});
        IEscswapV3Pool pool = IEscswapV3Pool(PoolAddress.computeAddress(factory, poolKey));
