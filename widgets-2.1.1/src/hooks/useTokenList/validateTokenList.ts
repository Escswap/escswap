import type { TokenInfo, TokenList } from '@escswap/token-lists'
...
const validator = new Promise<Ajv>(async (resolve) => {
  const [ajv, schema] = await Promise.all([import('ajv'), import('@escswap/token-lists/src/tokenlist.schema.json')])
