...
import '@escswap/v3-periphery/contracts/base/PeripheryImmutableState.sol';
import '@escswap/v3-core/contracts/libraries/SafeCast.sol';
import '@escswap/v3-core/contracts/libraries/TickMath.sol';
import '@escswap/v3-core/contracts/libraries/TickBitmap.sol';
import '@escswap/v3-core/contracts/interfaces/IEscswapV3Pool.sol';
import '@escswap/v3-core/contracts/interfaces/callback/IEscswapV3SwapCallback.sol';
import '@escswap/v3-periphery/contracts/libraries/Path.sol';
import '@escswap/v3-periphery/contracts/libraries/PoolAddress.sol';
import '@escswap/v3-periphery/contracts/libraries/CallbackValidation.sol';
import '@escswap/v2-core/contracts/interfaces/IEscswapV2Pair.sol';
...
contract MixedRouteQuoterV1 is IMixedRouteQuoterV1, IEscswapV3SwapCallback, PeripheryImmutableState {
    ...
    function getPool(
        ...
        address tokenB,
        uint24 fee
    ) private view returns (IEscswapV3Pool) {
        return IEscswapV3Pool(PoolAddress.computeAddress(factory, PoolAddress.getPoolKey(tokenA, tokenB, fee)));
    }
    ...
    function getPairAmountOut(
        ...
        (uint256 reserveIn, uint256 reserveOut) = EscswapV2Library.getReserves(factoryV2, tokenIn, tokenOut);
        return EscswapV2Library.getAmountOut(amountIn, reserveIn, reserveOut);
    }

    /// @inheritdoc IEscswapV3SwapCallback
    function escswapV3SwapCallback(
        ...
        IEscswapV3Pool pool = getPool(tokenIn, tokenOut, fee);
        ...
        )
    {
        ...
        IEscswapV3Pool pool = getPool(params.tokenIn, params.tokenOut, params.fee);
