...
import '@escswap/v3-periphery/contracts/base/BlockTimestamp.sol';
...
import '@escswap/v3-periphery/contracts/libraries/OracleLibrary.sol';
...
abstract contract OracleSlippage is IOracleSlippage, PeripheryImmutableState, BlockTimestamp {
    ...
    function getBlockStartingAndCurrentTick(IEscswapV3Pool pool)
        internal
        view
    ...
    function getPoolAddress(
        ...
        address tokenB,
        uint24 fee
    ) internal view virtual returns (IEscswapV3Pool pool) {
        pool = IEscswapV3Pool(PoolAddress.computeAddress(factory, PoolAddress.getPoolKey(tokenA, tokenB, fee)));
    }
        ...
        returns (int256 syntheticAverageTick, int256 syntheticCurrentTick)
    {
        ...
        for (uint256 i = 0; i < numPools; i++) {
            ...
            IEscswapV3Pool pool = getPoolAddress(tokenIn, tokenOut, fee);
            ...
            if (secondsAgo == 0) {
                ...
                (, currentTick, , , , , ) = IEscswapV3Pool(pool).slot0();
            }
