...
contract TokenValidator is ITokenValidator, IEscswapV2Callee, ImmutableState {
    string internal constant FOT_REVERT_STRING = 'FOT';
    // https://github.com/Escswap/v2-core/blob/1136544ac842ff48ae0b1b939701436598d74075/contracts/EscswapV2Pair.sol#L46
    ...
    function _validate(
        ...
        address pairAddress = EscswapV2Library.pairFor(this.factoryV2(), token, baseToken);
        ...
        (, bytes memory returnData) = address(pairAddress).call(abi.encodeWithSelector(IEscswapV2Pair.token0.selector));
        ...
        IEscswapV2Pair pair = IEscswapV2Pair(pairAddress);
    ...
    function escswapV2Call(
        address,
        uint256 amount0,
        uint256,
        bytes calldata data
    ) external view override {
        IEscswapV2Pair pair = IEscswapV2Pair(msg.sender);
