...
import { EscswapInterfaceMulticall } from '../types/v3/EscswapInterfaceMulticall';
import { EscswapInterfaceMulticall__factory } from '../types/v3/factories/EscswapInterfaceMulticall__factory';
import { ChainId } from '../util';
import { ESCSWAP_MULTICALL_ADDRESSES } from '../util/addresses';
...
export type EscswapMulticallConfig = {
  gasLimitPerCallOverride?: number;
};

/**
 * The EscswapMulticall contract has added functionality for limiting the amount of gas
 ...
 *
 * @export
 * @class EscswapMulticallProvider
 */
export class EscswapMulticallProvider extends IMulticallProvider<EscswapMulticallConfig> {
  private multicallContract: EscswapInterfaceMulticall;
  ...
  constructor(
    ...
    super();
    const multicallAddress = ESCSWAP_MULTICALL_ADDRESSES[this.chainId];

    if (!multicallAddress) {
      throw new Error(
        `No address for Escswap Multicall Contract on chain id: ${chainId}`
      );
    }

    this.multicallContract = EscswapInterfaceMulticall__factory.connect(
      multicallAddress,
      this.provider
    );
  }
