...
import EscswapV3Pool from '@escswap/v3-core/artifacts/contracts/EscswapV3Pool.sol/EscswapV3Pool.json'
import EscswapV3FactoryJson from '@escswap/v3-core/artifacts/contracts/EscswapV3Factory.sol/EscswapV3Factory.json'
import NFTDescriptorJson from '@escswap/v3-periphery/artifacts/contracts/libraries/NFTDescriptor.sol/NFTDescriptor.json'
...
import NonfungibleTokenPositionDescriptor from '@escswap/v3-periphery/artifacts/contracts/NonfungibleTokenPositionDescriptor.sol/NonfungibleTokenPositionDescriptor.json'
import SwapRouter from '@escswap/v3-periphery/artifacts/contracts/SwapRouter.sol/SwapRouter.json'
...
const v3CoreFactoryFixture: Fixture<IEscswapV3Factory> = async ([wallet]) => {
  return ((await waffle.deployContract(wallet, {
    bytecode: EscswapV3FactoryJson.bytecode,
    abi: EscswapV3FactoryJson.abi,
  })) as unknown) as IEscswapV3Factory
}

export const v3RouterFixture: Fixture<{
  weth9: IWETH9
  factory: IEscswapV3Factory
  router: ISwapRouter
...
export const escswapFactoryFixture: Fixture<EscswapFactoryFixture> = async (wallets, provider) => {
...
export type EscswapFixtureType = {
  factory: IEscswapV3Factory
  fee: FeeAmount
  nft: INonfungiblePositionManager
  pool01: string
  pool12: string
  poolObj: IEscswapV3Pool
  router: ISwapRouter
  staker: EscswapV3Staker
...
export const escswapFixture: Fixture<EscswapFixtureType> = async (wallets, provider) => {
  const { tokens, nft, factory, router } = await escswapFactoryFixture(wallets, provider)
  const signer = new ActorFixture(wallets, provider).stakerDeployer()
  const stakerFactory = await ethers.getContractFactory('EscswapV3Staker', signer)
  const staker = (await stakerFactory.deploy(factory.address, nft.address, 2 ** 32, 2 ** 32)) as EscswapV3Staker
  ...
  const poolObj = poolFactory.attach(pool01) as IEscswapV3Pool
...
export const poolFactory = new ethers.ContractFactory(EscswapV3Pool.abi, EscswapV3Pool.bytecode)
