...
import { escswapFixture, mintPosition, EscswapFixtureType } from '../shared/fixtures'
import {
  expect,
  getMaxTick,
  getMinTick,
...
describe('unit/Stakes', () => {
  ...
  let context: EscswapFixtureType
  ...
  beforeEach('create fixture loader', async () => {
    context = await loadFixture(escswapFixture)
  ...
  describe('#stakeToken', () => {
    ...
    describe('fails when', () => {
      it('deposit is already staked in the incentive', async () => {
        ...
        await subject(tokenId, lpUser0)
        await expect(subject(tokenId, lpUser0)).to.be.revertedWith('EscswapV3Staker::stakeToken: token already staked')
      })
      ...
      it('you are not the owner of the deposit', async () => {
        ...
        await expect(subject(tokenId, actors.lpUser2())).to.be.revertedWith(
          'EscswapV3Staker::stakeToken: only owner can stake token'
        )
      })
      ...
      it('has 0 liquidity in the position', async () => {
        ...
        await expect(subject(tokenId2, lpUser0)).to.be.revertedWith(
          'EscswapV3Staker::stakeToken: cannot stake token with 0 liquidity'
        )
      })
      ...
      it('token id is for a different pool than the incentive', async () => {
        ...
        ).to.be.revertedWith('EscswapV3Staker::stakeToken: token pool is not the incentive pool')
      })
      ...
      it('incentive key does not exist', async () => {
        ...
        ).to.be.revertedWith('EscswapV3Staker::stakeToken: non-existent incentive')
      })

      it('is past the end time', async () => {
        await Time.set(timestamps.endTime + 100)
        await expect(subject(tokenId, lpUser0)).to.be.revertedWith('EscswapV3Staker::stakeToken: incentive ended')
      })
      ...
      it('is before the start time', async () => {
        ...
        await expect(subject(tokenId, lpUser0)).to.be.revertedWith('EscswapV3Staker::stakeToken: incentive not started')
      })
    })
  })
  ...
  describe('#getRewardInfo', () => {
    ...
    it('reverts if stake does not exist', async () => {
      ...
      await expect(context.staker.connect(lpUser0).getRewardInfo(stakeIncentiveKey, '100')).to.be.revertedWith(
        'EscswapV3Staker::getRewardInfo: stake does not exist'
      )
    })
  })
  ...
  describe('#unstakeToken', () => {
    ...
    describe('fails if', () => {
      it('stake has already been unstaked', async () => {
        await Time.setAndMine(timestamps.endTime + 1)
        await subject(lpUser0)
        await expect(subject(lpUser0)).to.revertedWith('EscswapV3Staker::unstakeToken: stake does not exist')
      })

      it('you have not staked', async () => {
        await expect(subject(actors.lpUser2())).to.revertedWith(
          'EscswapV3Staker::unstakeToken: only owner can withdraw token'
        )
      })
      ...
      it('non-owner tries to unstake before the end time', async () => {
        ...
        await expect(subject(nonOwner)).to.revertedWith('EscswapV3Staker::unstakeToken: only owner can withdraw token')
