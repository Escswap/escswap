...
import { escswapFixture, EscswapFixtureType } from '../shared/fixtures'
import {
  expect,
  getMaxTick,
  getMinTick,
    ...
    describe('fails when', () => {
      describe('invalid timestamps', () => {
        ...
        it('end time is before start time', async () => {
          ...
          await expect(subject(params)).to.be.revertedWith(
            'EscswapV3Staker::createIncentive: start time must be before end time'
          )
        })
      ...
      describe('invalid reward', () => {
        it('totalReward is 0 or an invalid amount', async () => {
          ...
          ).to.be.revertedWith('EscswapV3Staker::createIncentive: reward must be positive')
        })
      })
    })
  })
    ...
    describe('reverts when', async () => {
      it('block.timestamp <= end time', async () => {
        ...
        await expect(subject({})).to.be.revertedWith(
          'EscswapV3Staker::endIncentive: cannot end incentive before end time'
        )
      })
      ...
      it('incentive does not exist', async () => {
        ...
        ).to.be.revertedWith('EscswapV3Staker::endIncentive: no refund available')
      })
